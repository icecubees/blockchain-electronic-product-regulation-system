...
function renderRiskPill(level, tag) {
  ...
  return (
    <span key={tag} className={`rounded-full px-3 py-1 text-xs font-semibold ${styles[level] || "bg-slate-100 text-slate-700"}`}>
      {tag}
    </span>
  );
}
    ...
    <div className="min-h-screen bg-slate-100 px-4 py-10">
      <div className="mx-auto max-w-5xl overflow-hidden rounded-3xl bg-white shadow-xl">
        <div className="bg-slate-900 px-8 py-8 text-white">
          ...
          <p className="mt-2 text-slate-300">当前二维码不包含商品 ID，因此只能展示旧版静态字段。</p>
        </div>
      ...
      <div className="mx-auto max-w-6xl space-y-6">
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="mt-2 text-slate-500">报告整合数据库记录、审计日志、IPFS 证据与链上状态信息。</p>
          </div>
          <Link
            to="/home"
          ...
          <>
              ...
              <div className="rounded-3xl border border-amber-200 bg-amber-50 px-6 py-5 shadow-sm">
                ...
                <div className="mt-2 flex flex-wrap items-center gap-2">
                  <span className="text-sm font-semibold text-amber-800">
                    风险等级：{traceData.riskProfile.riskLevel || "未提供"}
                  </span>
              ...
              <div className="grid gap-6 p-8 lg:grid-cols-3">
                <section className="space-y-4">
                  ...
                  <div className="space-y-4 rounded-2xl bg-slate-50 p-5">
                    <InfoRow label="类别" value={CATEGORY_LABELS[traceData.category] || traceData.category} />
                    <InfoRow label="品牌" value={traceData.brand} />
                    <InfoRow label="型号" value={traceData.model} />
                    <InfoRow label="序列号 / IMEI" value={traceData.serialNumberMasked} />
                    <InfoRow label="批次号" value={traceData.batchNo} />
                ...
                <section className="space-y-4">
                  ...
                  <div className="space-y-4 rounded-2xl bg-slate-50 p-5">
                    ...
                    <InfoRow
                      label="电池健康度"
                      value={traceData.batteryHealth !== null && traceData.batteryHealth !== undefined ? `${traceData.batteryHealth}%` : "未提供"}
                    />
                    <InfoRow label="配件情况" value={traceData.accessoryStatus} />
                    <InfoRow label="CCC 编号" value={traceData.cccNumber} />
                    ...
                    <InfoRow label="电池安全" value={formatBoolean(traceData.compliance?.batterySafetyPassed)} />
                    <InfoRow label="充电安全" value={formatBoolean(traceData.compliance?.chargerSafetyPassed)} />
                    <InfoRow label="外观等级" value={traceData.compliance?.appearanceGrade} />
                    <InfoRow label="功能测试" value={formatBoolean(traceData.compliance?.functionalTestPassed)} />
                    <InfoRow label="已声明维修历史" value={formatBoolean(traceData.compliance?.repairHistoryDeclared)} />
                  </div>
                </section>

                <section className="space-y-4">
                  <div className="text-sm font-semibold uppercase tracking-wide text-slate-500">监管与链上状态</div>
                  <div className="space-y-4 rounded-2xl bg-slate-50 p-5">
                    <div className="flex items-center justify-between">
                    ...
                    <InfoRow label="当前库存" value={String(traceData.stock)} />
                    ...
                    <InfoRow
                      label="风险等级"
                      value={RISK_LEVEL_LABELS[traceData.chainSummary?.riskLevel] || String(traceData.chainSummary?.riskLevel ?? "未提供")}
                    />
                    <InfoRow label="链上召回标记" value={traceData.chainSummary?.recallFlag ? "是" : "否"} />
                    <InfoRow label="卖家信誉分" value={String(traceData.seller?.reputationScore ?? "未提供")} />
                    <InfoRow label="卖家是否拉黑" value={traceData.seller?.isBlacklisted ? "是" : "否"} />
                  </div>
                </section>
              </div>
              ...
              <div className="grid gap-6 px-8 pb-8 lg:grid-cols-2">
                <div className="rounded-2xl bg-slate-50 p-5">
                  ...
                  <div className="flex flex-wrap gap-3">
                    ...
                    {traceData.qualificationHash && traceData.qualificationHash !== "NoCert" && (
                      <a
            ...
            <div className="rounded-3xl bg-white p-8 shadow-xl">
              <div className="mb-6">
                ...
                <p className="mt-2 text-slate-500">这些订单记录展示了商品上架后的流转与履约过程。</p>
              </div>
                    ...
                    <div key={order.id} className="rounded-2xl border border-slate-200 bg-slate-50 p-5">
                      <div className="flex flex-wrap items-center justify-between gap-3">
                        <div>
                          ...
                          <div className="mt-1 text-sm text-slate-500">
                            买家：{order.buyer?.username || "未知"} | 创建时间：{formatDate(order.createdAt)}
                          </div>
                        </div>
                        <StatusBadge
                      ...
                      {(order.comment || order.complaintReason || order.rulingDetails || order.sellerResponse) && (
                        <div className="mt-4 space-y-2 text-sm text-slate-700">
                          {order.comment && <div><span className="text-slate-500">评价：</span>{order.comment}</div>}
                          ...
                          {order.sellerResponse && (
                            <div><span className="text-slate-500">卖家答辩：</span>{order.sellerResponse}</div>
                          )}
                          {order.rulingDetails && (
                            <div><span className="text-slate-500">裁决：</span>{order.rulingDetails}</div>
                          )}
                        </div>
                      )}
                    </div>
            ...
            <div className="rounded-3xl bg-white p-8 shadow-xl">
              <div className="mb-6">
                ...
                <p className="mt-2 text-slate-500">该时间线将商品、订单与争议事件串联为一条完整追溯链路。</p>
              </div>
            ...
            <div className="rounded-3xl bg-white p-8 shadow-xl">
              <div className="mb-6">
                ...
                <p className="mt-2 text-slate-500">保修、维修、更换与退款记录共同补全设备治理生命周期。</p>
              </div>
